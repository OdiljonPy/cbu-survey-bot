...
async def create_more_answer_btn(user_id: int, question_id: int, lang: str) -> InlineKeyboardMarkup:
    ...
    btn.add(InlineKeyboardButton(text=f'{enter}', callback_data=f"more_answer_done:{question_id}"))
    btn.adjust(*[1])
    return btn.as_markup()


async def create_more_answer_btn_long(user_id: int, question_id: int, lang: str) -> InlineKeyboardMarkup:
    btn_list = await get_question_answers(question_id, user_id)
    btn = InlineKeyboardBuilder()
    for text in btn_list:
        answer_id = btn_list.index(text)
...
async def create_inline_btn(user_id: int, question_id: int) -> dict:
    ...
    if await is_long_answers(question_id):
        ...
        if await is_more_answers(question_id):
            btn = await create_more_answer_btn_long(user_id, question_id, lang)
        else:
    ...
    if await is_more_answers(question_id):
        btn = await create_more_answer_btn(user_id, question_id, lang)
    else:
